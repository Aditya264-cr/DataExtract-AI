...
export const RightSidebar: React.FC<RightSidebarProps> = ({ originalData, editedData, isOpen, onToggle }) => {
    ...
    const handleExport = (format: 'json' | 'csv' | 'txt' | 'xlsx' | 'pdf') => {
        ...
        if (format === 'json') {
            ...
            if (settings.showWatermark) {
                XLSX.utils.sheet_add_aoa(ws, [[watermarkText]], { origin: -1 });
            }
            XLSX.utils.book_append_sheet(wb, ws, "Extracted Data");

            // 2. Separate Sheets for Detected Tables
            const tables = editedData.structuredData.tables || [];
            tables.forEach((table, idx) => {
                if (table.rows && table.rows.length > 0) {
                    const tableData = table.rows.map(row => {
                        const cleanRow: any = {};
                        Object.entries(row).forEach(([k, v]: [string, any]) => {
                            cleanRow[k] = v?.value ?? v; // Handle extracted field object or direct value
                        });
                        return cleanRow;
                    });
                    const wsTable = XLSX.utils.json_to_sheet(tableData);
                    // Sheet names limited to 31 chars
                    const sheetName = (table.tableName || `Table ${idx + 1}`).substring(0, 31);
            ...
            // Key-Value Table
            const flattened = flattenObject(editedData);
            // Filter out table placeholder strings for the PDF KV list
            ...
            if (kvEntries.length > 0) {
                ...
                autoTable(doc, {
                    ...
                    columnStyles: { 0: { fontStyle: 'bold', cellWidth: 60 } }, // Bold keys
            ...
            tables.forEach(table => {
                ...
                if (headers.length > 0) {
                    const body = table.rows.map(row => headers.map(h => {
                        const cell = row[h];
            ...
            if (settings.showWatermark) {
                ...
                for (let i = 1; i <= totalPages; i++) {
                    ...
                    doc.text(watermarkText, 14, pageHeight - 10);
                }
            }
                    ...
                    <div className={`transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${isOpen ? 'mr-3' : 'w-full flex justify-center'}`}>
                        <Tooltip text={isOpen ? "Collapse Panel" : "Expand Studio"} position={isOpen ? "bottom" : "left"}>
                            <button 
                                ...
                                className="p-2 rounded-xl text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-zinc-800 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none transition-all flex-shrink-0"
                            >
